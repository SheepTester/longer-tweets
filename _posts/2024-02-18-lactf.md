...
`local_238`{:.language-c} has the user input, and `sVar2`{:.language-c} is its length (which must be greater than 5 characters).

Inside the do-while loop, `pcVar3`{:.language-c} seems to be the equivalent of the `i`{:.language-c} of a for loop because it increments every iteration. It's a pointer to the current character in the loop and uses `strncmp`{:.language-c} to compare the next 6 characters from the pointer to the strings `pretty` and `please`, incrementing `iVar5`{:.language-c} and `iVar4`{:.language-c} if they match, respectively. The loop seems to stop 6 characters before the end of the string (with the `sVar - 6`{:.language-c}). Therefore, this loop counts the number of `pretty`s and `please`s in the input string.
...
Then, after the loop, right before printing the contents of `flag.txt`, it checks if `(iVar5 + iVar4 == 0x36) && (iVar5 - iVar4 == -0x18)`{:.language-c}. This is a system of equations:
...
Why didn't that work? Well, there's also an if statement that checks that `strstr(&local_238,"flag")`{:.language-c} isn't `0x0`{:.language-c}; otherwise, it gives the error we got. [`strstr`{:.language-c}][strstr] finds the second string, `flag`{:.language-c}, in the first string, which is my input, and returns null (`0x0`{:.language-c}) if it can't find it. So this means I just need to add `flag`{:.language-c} to my input:
...
The website is a roommate finder[^1]. On the "Find Roomates" page, you search for users by their username, then send an invite to them. The "View Invitations" page lists the invites that were sent to you.
...
Both of these pages use `innerHTML`{:.language-js} without any sanitization, so a standard XSS payload like `<img src=x onerror="alert(1)">`{:.language-html} should work. This tries to load a nonexistent image at `./x` and fails, running the JavaScript in the `onerror` handler.
...
The following HTML makes the same POST request using [JavaScript `fetch`{:.language-js}][fetch], except it invites the username of one of my accounts, `a`, instead.
...
It seems by using JavaScript's `FormData`{:.language-js} object, it always wraps its data in some `WebKitFormBoundary`, which the server doesn't seem to accept. So I got rid of it and set the `Content-Type` header manually instead:
...
Most of the telltale clues are in the `runCode`{:.language-js} definition in index.js, which ultimately just runs `eval`{:.language-js} on whatever code you submit (so you can do anything you want on their server). However, there are two catches:
...
This means `code`{:.language-js} must only use `()+[]!`. This is infamously [JSFuck][jsfuck]'s character set, where any JavaScript can be rewritten using just those characters and still be valid JavaScript. The [JSFuck website][jsfuck] has a converter for arbitrary JS into equivalent JavaScript that only uses the characters in `()+[]!`.
...
Normally, JavaScript arrays, when cast to a string, will cast each of its elements to a string and then join them together with a comma `,`. For example, `'' + [1, 2, 3]`{:.language-js} produces the string `1,2,3`. JSFuck uses this to convert values into strings. However, the effect of this change is that casting arrays to strings always returns the string `^w^` instead of the contents of the array.

What does this change? Well, JSFuck relies on JavaScript's current array-to-string behavior for two important values:

- Producing zero. Currently, casting an empty array `[]`{:.language-js} to a string produces the empty string. An empty string cast to a number is 0. Since using `+`{:.language-js} as a unary operator only makes sense for numbers, JavaScript will cast values to a number if you use `+`{:.language-js} on it. So, `+[]`{:.language-js} produces 0 in JavaScript.

  But now that `[]`{:.language-js} becomes `^w^`, `+[]`{:.language-js} becomes `+'^w^'`{:.language-js}. `^w^` is not a number<sup>[citation needed]</sup>, so this becomes `NaN`{:.language-js} instead of 0.

  Fortunately, it's easy enough to work around this:
  1. Cast `NaN`{:.language-js} into a boolean using `!`{:.language-js} (the logical NOT operator). `NaN`{:.language-js} is falsy, so it becomes `false`{:.language-js}.
  2. Cast `false`{:.language-js} to a number, producing 0.

  Here's how the new expression gets evaluated: `+!!+[]`{:.language-js} → `+!!+'^w^'`{:.language-js} → `+!!NaN`{:.language-js} → `+!true`{:.language-js} → `+false`{:.language-js} → `0`{:.language-js}

  Fortunately, non-zero numbers still work normally. JSFuck takes advantage of 0 being falsy, and inverting it makes it `true`{:.language-js}. Casting `true`{:.language-js} to a number produces 1, which you can add to itself to produce the natural numbers: `+!+[]`{:.language-js} is 1, `+!+[]+!+[]`{:.language-js} is 2, and so on. Since `NaN`{:.language-js} is also falsy, `!+[]`{:.language-js} still becomes `true`{:.language-js}.

- Casting to a string. Since an empty array becomes an empty string, you can add it to any value to turn it into a string. `![]+[]`{:.language-js} becomes the string `'false'`{:.language-js}, and so on.

  With the `^w^` change, `![]+[]`{:.language-js} becomes `false^w^`. This doesn't break too many things because much of JSFuck involves getting letters of keywords, like doing `(![]+[])[3]`{:.language-js} for the letter `s`; adding `^w^` to the end of the string won't affect this.

  However, this change does affect numbers. JSFuck's converter in particular prefers values like `10` over `9`, which is a smaller value but requires more code because the sum of its digits is larger. This is because producing 10 only involves concatenating 1 and 0, then casting it to a number (`+(+!+[] + [+[]])`{:.language-js} → `+(1 + [0])`{:.language-js} → `+(1 + '0')`{:.language-js} → `+'10'`{:.language-js} → `10`{:.language-js}), while producing 9 requires adding 1 to itself nine times.

  Now, after the change, even by replacing `+[]`{:.language-js} with `+!!+[]`{:.language-js} as discussed above, `+(+!+[] + [+!!+[]])`{:.language-js} evaluates to `+(1 + [0])`{:.language-js} → `+(1 + '0^w^')`{:.language-js} → `+'10^w^'`{:.language-js} → `NaN`{:.language-js}. Sad!
...
My first attempt was to try resetting `[].__proto__.toString`{:.language-js} back to its original form, so I put it into JSFuck's converter and tried to fix any issues caused by the change. These were the substitutions I did to the code generated by JSFuck:

- `[0]`{:.language-js}: `[+[]]` → `[+!!+[]]`{:.language-js}

  0 was most often used to index into strings, which is why it's in square brackets.

- 10: `+!+[]+[+!!+[]]`{:.language-js} → `!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]`{:.language-js}

- `[11]`{:.language-js}: `[+!+[]+[+!+[]]]`{:.language-js} → `[!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]]`{:.language-js}

  This was used for getting the letter `m` for some other thing.

- 12: `+!+[]+[!+[]+!+[]]`{:.language-js} → `!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]`{:.language-js}

- `i`: `([![]]+[][[]])[!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]]`{:.language-js} → `([][[]]+[])[!+[]+!+[]+!+[]+!+[]+!+[]]`{:.language-js}

  The original way used `'falseundefined'[10]`{:.language-js} because it probably produced shorter code, but now it becomes `'^w^undefined'[10]`{:.language-js}, which is `undefined`{:.language-js} and usually results in a TypeError. I replaced it with `'undefined^w^'[5]`{:.language-js}.

- `/`: `(![]+[+!!+[]])[([][[]]+[])[!+[]+!+[]+!+[]+!+[]+!+[]]+(!![]+[])[+!!+[]]+(![]+[])[+!+[]]+(![]+[])[!+[]+!+[]]+([][[]]+[])[!+[]+!+[]+!+[]+!+[]+!+[]]+([][(![]+[])[+!!+[]]+(![]+[])[!+[]+!+[]]+(![]+[])[+!+[]]+(!![]+[])[+!!+[]]]+[])[!+[]+!+[]+!+[]]+(![]+[])[!+[]+!+[]+!+[]]]()[!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]]`{:.language-js} → `(![]+[+!!+[]])[([][[]]+[])[!+[]+!+[]+!+[]+!+[]+!+[]]+(!![]+[])[+!!+[]]+(![]+[])[+!+[]]+(![]+[])[!+[]+!+[]]+([][[]]+[])[!+[]+!+[]+!+[]+!+[]+!+[]]+([][(![]+[])[+!!+[]]+(![]+[])[!+[]+!+[]]+(![]+[])[+!+[]]+(!![]+[])[+!!+[]]]+[])[!+[]+!+[]+!+[]]+(![]+[])[!+[]+!+[]+!+[]]]()[!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]+!+[]]`{:.language-js}

  JSFuck gets `/` by calling [`'false0'.italics()`{:.language-js}][italics], which wraps it into `<i>false0</i>`{:.language-html}. This string was probably selected for its shorter code length, but because it relies on concatenation it becomes `<i>false^w^</i>`{:.language-html}, which changes the index of `/` in the string.

  What's `/` for? JSFuck uses it to create a function[^10] with body `return/false/`{:.language-js}, which returns an instance of `RegExp`{:.language-js}. It can then do `/false/.constructor`{:.language-js} to get the `RegExp`{:.language-js} class constructor itself, then construct `RegExp('/')`{:.language-js} which creates a regular expression that just has a `/`. However, in JavaScript's regular expression literals, `/` must be escaped, so casting `RegExp('/')`{:.language-js} to a string produces a string containing `/\//`{:.language-js}.

  So `Function('return/false/').constructor('/') + ''`{:.language-js} is how JSFuck gets a backslash character `\`.

[italics]: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/italics

All this was just to generate the underscore character `_` for `__proto__`{:.language-js}. JSFuck creates a function that returns a string with an octal escape sequence, then calls it: `Function('return"\137"')()`{:.language-js}. This escape sequence method is JSFuck's last resort for characters it can't get through other means.

[^10]: You might be wondering how JSFuck can create a function in the first place. You can get `a` from `false`{:.language-js} and `t` from `true`{:.language-js}, and arrays have a method named `at`, which you can access with `[]['a' + 't']`{:.language-js}. It's also possible to get the other letters in `constructor`{:.language-js}, so you can use `[]['at']['constructor']`{:.language-js} to get a reference to the `Function`{:.language-js} constructor, which you can use to convert strings to functions. For example, `Function('return 3')`{:.language-js} creates a function that returns 3.

After fixing everything, I realized that my plan of resetting `[].__proto__.toString`{:.language-js} wouldn't work because I also needed a reference to the original `Array.prototype.toString`{:.language-js}. While I felt like it was possible, I couldn't be bothered to try to investigate and fix more issues caused by an even longer string needed.
...
1. Store the JSFuck code for a backslash character in a variable `backslash`{:.language-js} for convenience.

2. Split the attack code I want to execute (inside `Array.from()`{:.language-js}) into individual characters, convert the characters into an octal escape sequence (`backslash`{:.language-js} + the code point of the character in base 8), and then join them all together with `+`{:.language-js} signs. This sets `w`{:.language-js} to code to generate a long string of octal sequences.

   For example, the string `hello` would become `'\' + 150 + '\' + 145 + '\' + 154 + '\' + 154 + '\' + 157`{:.language-js}.

3. Wrap `w`{:.language-js} in code that puts the octal sequences in a string literal. This is essentially the same as

   ```js
   w = `return "${w}"`
   ```

4. Wrap `w`{:.language-js} in code that converts the string into a function and calls it.
...
5. Wrap `w`{:.language-js} again to actually call my script:
...
Even though it didn't look like it in index.js, `eval`{:.language-js} seemed to be sandboxed somehow, and `require`{:.language-js} wasn't directly available.
...
This gets a `Promise`{:.language-js} object's constructor (`Promise`{:.language-js})'s constructor (`Function`{:.language-js}) to create a function that returns `process`{:.language-js}, then access `require`{:.language-js} via `process.mainModule.require`{:.language-js}.
...
Here are links to [lactf-jsfudge.js][gen] (392 kB) and the [code that generates it][gener].
...
(That `e = […]`{:.language-py} line is a [long 3D array of numbers][e] ranging from 10 to 17.)

[e]: https://github.com/uclaacm/lactf-archive/blob/main/2024/rev/glottem/glottem#L10

The `#!/bin/sh`{:.language-sh} line suggests that it's a Bash script (more accurately, just `sh` for shell). But other lines in the code, as hinted by the `node` and `python3` commands, suggest that it might also be a JavaScript and Python file. It's a [polyglot] (hence the challenge name)!
...
My impression of this is that `4201337` is just a string to indicate the start and end of a raw string, so you don't have to worry about escaping any characters inside there. I thought that maybe `<<`{:.language-sh} is like how `>>`{:.language-sh} is to `>`{:.language-sh}, where it pipes the contents of some file with that name into a command `1`{:.language-sh}, which is an alias for true. While writing this write-up, I looked up what `<<`{:.language-sh} does, and I appear to be completely wrong. Oh well.
...
1. The first command stores a list in `e`{:.language-sh}, which isn't used anywhere, so it doesn't do anything.
...
The numbers, `1<<4201337`{:.language-js}, `1`{:.language-js}, and `4201337`{:.language-js} don't do anything. While JavaScript's comment syntax normally uses `//`{:.language-js}, it will also treat `#!`{:.language-sh} as a comment if it's at the beginning of the file.[^11]

[^11]: This was originally a Node-specific feature that has recently been [standardized into JavaScript][shebang], adding a fourth comment syntax to JavaScript's existing family of `//`{:.language-js}, `/* */`{:.language-js}, and `<!-- -->`{:.language-html}.
...
Interestingly, JavaScript doesn't use `e`{:.language-js} either. It creates aliases for `exit`{:.language-js} and `argv`{:.language-js} so some of the statements can be the same in Python and JavaScript.

`s`{:.language-js} is the flag (including the `lactf{...}` part), and it loops over each character's code point and does some math with it. The `%93097`{:.language-js} looks a bit annoying because modular arithmetic is hard to reverse.

The final line, `exit(+(d!=260,[d!=61343])[0])`{:.language-js}, is interesting, and I'll ramble more about it when looking at Python. But ultimately, `d`{:.language-js} just needs to equal `61343`{:.language-js} at the end for the script to exit with 0, which means success in Bash.

I tried to work backwards. Even though I don't know what `d`{:.language-js} was, I knew that it couldn't be more than 93097, and before being added to the character code, it would be multiplied by 31. So, I thought, maybe I could try guessing all the characters in `alpha`{:.language-js} and see which of them have a prior value of `d`{:.language-js} that could result in `61343`{:.language-js}.
...
Again, the numbers (`1<<4201337`{:.language-py}, `1//1`{:.language-py}, and `4201337`{:.language-py}), tuples, and multiline strings `"""..."""`{:.language-py} don't do anything, and `#`{:.language-py} is Python syntax for comments. So the only lines that matter are,
...
This one does use `e`{:.language-py}. The `range(6,len(s)-2)`{:.language-py} makes it iterate over adjacent pairs of characters inside the curly braces of `lactf{...}`. `e`{:.language-py} has a 2D array for every character in the flag, and it looks up numbers inside it based on the characters of the pair. These numbers must add up to `260`{:.language-py}.
...
In JavaScript, the comma operator runs and discards the result of `d!=260`{:.language-js}, so `(d!=260,[d!=61343])`{:.language-js} returns the array `[d!=61343]`{:.language-js}; `[0]`{:.language-js} gets the boolean inside the array, and `+`{:.language-js} casts it to a number for the exit code. So JavaScript exits with `d!=61343`{:.language-js}.

Meanwhile, in Python, the comma operator forms a tuple, so `[0]`{:.language-py} indexes into the _tuple_ `(d!=260,[d!=61343])`{:.language-py}, returning `d!=260`{:.language-py}. Python exits with `d!=260`{:.language-py}. I think it's pretty cool that the polyglot exploits the difference in how `,` behaves in the two languages.

Anyways, I figured that since `len(e)`{:.language-py} was 26, and the smallest value in `e`{:.language-py} was 10, the content of the string between the curly braces of the flag was probably 26 characters long too, and I needed to find a string whose pairs only had 10 values inside `e`{:.language-py}. However, there seemed to be many such values in the array, so I felt like backtracking here wouldn't be helpful.
...
I tried working backwards using `e`{:.language-js}:
...
This looks for every occurrence of 10 in the last element of `e`{:.language-js} and looks up the first character of the pair based on the indices. It keeps track of these potential characters for the previous element of `e`{:.language-js}. It repeats the process for previous characters, except it only considers character pairs where the second character was a potential candidate from the last round.

This was the output. Each row represents potential candidate letters from each 2D array in `e`{:.language-js}.
...
`extend`{:.language-js} is a function that takes `A`{:.language-js}, a list of possible flag strings, and `et`{:.language-js}, the current 2D array from `e`{:.language-js}. It starts with the last pair, `extend([''], e.at(-1))`{:.language-js}, then grows it about another 8 times.
...
Looking at the provided [server.js][server], there are a few things to note:
...
- The OTP isn't stored anywhere unless Admin Bot accesses the page (identified by a special token in its cookie). In that case, the OTP is saved in a `Map`{:.language-js} between usernames and OTPs.
- On a `/flag` page, which takes both a username `user` and the OTP `otp`, if the corresponding entry in the `Map`{:.language-js} matches the given OTP, then the server will directly give the flag.
...
- Taking advantage of CSS attribute selectors like `[value^="hello"]`{:.language-css} or `[value*="hey"]`{:.language-css} to test if the input value starts with or contains a certain string, respectively.
- Loading font files based on the characters used in the page.

Looking at [note.js][note], the web page accepts an additional URL parameter `page`, a URL (which could be a `data:` URL) that it fetches and inserts into the document.
...
It inserts the contents of the provided URL using `innerHTML`{:.language-js}, which means I can perform my injection here. Curiously, it rejects `message`{:.language-js} if it's more than 6 MB, and annoyingly it also removes all `<style>`{:.language-html} tags.
...
- I can't load plugins with `<object>`{:.language-html} and `<embed>`{:.language-html}.
- I can't use `<base>`{:.language-html}, which changes the base URL that all relative paths on the page and relative to.
- I can't submit forms.
- I can't use `onerror` attributes and other event handlers to run JavaScript; XSS is infeasible.
- I can't use `<link>`{:.language-html} tags to load external CSS to get around the removal of `<style>`{:.language-html} tags.

Curiously (with help from ChatGPT to brainstorm what the CSP doesn't over), I still can load images and `<iframe>`{:.language-html}s. However, my external `<iframe>`{:.language-html}s can't really communicate with or access anything on the web page with the OTP, so I didn't know how to use them.
...
While researching ways to get `<iframe>`{:.language-html}s to work or add selectors to `style` attributes, I found a post[^12] that had something like this:

[^12]: I tried looking through my browsing history, but there's no evidence of me ever learning about this from anything online. Based on my browsing history, it just looks like I was trying things like `<span id="document">`{:.language-html} then suddenly had a stroke of genius and tried doing `<form name=querySelectorAll>`{:.language-html}. Then, I wondered where I got this idea from and tried to look up "form name getelementbyid injection" to find the original post, but seemed to fail.
...
I was trying to get rid of `document.querySelectorAll`{:.language-js} using just HTML so it couldn't remove my `<style>`{:.language-html} elements. I knew that IDs create global variables that reference the element (for example, `<span id="hey">`{:.language-html} creates a global variable `hey`{:.language-js}). However, it seems the built-in `document`{:.language-js} object has priority over `<span id="document">`{:.language-html}.

It seems, based on this post, that using a `name` attribute similarly creates a new entry under `document`{:.language-js}. For example, `<form name="hey">`{:.language-html} creates a new entry `document.hey`{:.language-js}, and apparently `<form name="getElementById">`{:.language-html} had more priority than the built-in `document.getElementById`{:.language-js} method.

So just by adding some HTML, I could replace `document.querySelectorAll`{:.language-js} with a form of the same name. This breaks the code because `document.querySelectorAll`{:.language-js} as a form isn't a function anymore, so calling it throws a TypeError, and `<style>`{:.language-html} elements are no longer removed. 🙌
...
- `[attr="value"]`{:.language-css} Whether the attribute is exactly the string.
- `[attr^="value"]`{:.language-css} Whether the attribute starts with a string.
- `[attr$="value"]`{:.language-css} Whether the attribute ends with a string.
- `[attr*="value"]`{:.language-css} Whether the attribute contains the string anywhere, at least once.
- `[attr*="value" i]`{:.language-css} Adding `i` at the end makes the comparison case insensitive.
- `[attr|="value"]`{:.language-css} This also can compare strings, but it has a special behavior that allows `lang="en"` and `lang="en-US"` to be treated similarly. It's not particularly useful for anything but the `lang` attribute.
...
Because of the CSP, I can't `<link>`{:.language-html} or `@import`{:.language-css} a CSS file from a custom web server that streams new rules over as characters get discovered one by one.
...
It builds a map `next`{:.language-js} of strings to strings. For example, `uaz` becomes `ua -> z` and `u -> az`. The latter is there just in case some trigraphs didn't make it due to the 6 MB limit, duplicates, or CSS variable collisions, but it's not great for making certain decisions.

Then, it starts with the first character and tries to build on it using `next`{:.language-js}. However, it kept failing with `i give up :(`, meaning that it reached a point where no trigraph could continue the string.
...
1. Creates edges between characters if a pair says one character is followed by another. This helps to show what letters can be chained.
2. Find an Eulerian path---a path that uses all the edges (pairs) in the graph. This ensures that every pair is used exactly once.
...
Earlier, I mentioned that CSS could do case-insensitive matching too, like `[value*="string" i]`{:.language-css}. I could almost halve the number of trigraphs by case-insensitively matching only digits and lowercase letters, no uppercase.
...
With 1000 CSS variables, the probability this can happen is 3.9%. That's incredibly low, so I should expect a collision most of the time. With 10 000 variables, the probability jumps up to 73%, which is good enough for me. This is why there's a `const N = 10000`{:.language-js} line in the script above.
...
Then, for each trigraph, it looks up the pairs[^14] that could constitute it, then uses the letter cases in the pairs to create the trigraph. However, multiple pairs might only differ by case, like `ab` and `Ab`. In these cases, my server just tries every combination; there usually aren't that many (maybe up to 16 combinations).
...
I did notice that when I decreased `N`{:.language-js} (the number of CSS variables) down to 1000, Admin Bot did start sending requests to my server. This observation was consistent with my hypotheses. But 1000 CSS variables isn't enough for me.
...
Removing the variable initializations broke my code because when a CSS variable isn't defined, I think it defaults to an empty string, resulting in duplicate commas like `background: ,url(...),;`{:.language-css}, which is not valid syntax. So I instead used `var(--name, none)`{:.language-css}, where `none` is the default value if the variable isn't defined. I closed my `<form>`{:.language-html} and `<style>`{:.language-html} tags, which I hadn't done before to save characters (it worked without the closing tags).
...
I created a route on my server that would keep loading by receiving a request and pretending to be working on it[^15], while in reality, it ignores the request.
...
As it turns out, my solution was the intended solution. On the Discord server, apparently some other teams used the bfcache to avoid refreshing the OTP. Chrome now avoids reloading the page when you go back and forth in history, but it apparently does re-execute the page, so it can fetch `page` again. This allows them to change their CSS file as they discover more characters, using the more traditional CSS injection approach. Another team claimed to take advantage of `Math.random()`{:.language-js}'s PRNG-ness, but I'm not sure if they've elaborated on that.

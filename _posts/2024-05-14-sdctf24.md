...
- UTF-32: Use 4 bytes per character, and encode each code value as a 32-bit integer. [Rust's `char`{:.language-rust} type](https://doc.rust-lang.org/std/primitive.char.html) uses UTF-32.
...
But as I wrote the server, I realized that it didn't really make sense for me to have to use a polyfilled `TextEncoder`{:.language-js} for the server since the language already handles that automatically everywhere. It would look suspect if I did something like `new Response(encoder.encode(html))`{:.language-js} instad of just `new Response(html)`{:.language-js}.
...
I decided to leave that up to [a library](https://www.npmjs.com/package/vlq), which happened to have a [helpful page explaining what the numbers meant](https://github.com/Rich-Harris/vlq/tree/master/sourcemaps). I decided to just map the entire minified line with my code (and other unrelated lines) to the entirety of the source code. This essentially means using `[]`{:.language-js} for all lines of the minified index.js except for the line I want, and then use `[0, 0, 0, 0]`{:.language-js} for that line:

- The first zero means the mapping for the minified file is for the entire line (column 0 to the end).
- The second zero refers to the first source file defined in the source map's `sources`{:.language-js} array.
...
However, this meant that when I follow the `console.log`{:.language-js} statements from the console to the source tab (in both Firefox and Chrome), it would always highlight the first line of the source file rather than the fake `console.log`{:.language-js} statement. But I decided people probably wouldn't pay close attention to that.
...
Finally, I can set a breakpoint in the form `submit` event handler, try checking a flag on some random input, then evaluate the right-hand side of the `===`{:.language-js} expression.
...
Learning Python, I've always been perturbed by many "features" (I don't like Python). For example, Python always likes abbreviating their names to the point where they don't really make sense at first class. For example, the `s` in `json.dumps`{:.language-py} means string. Of course.

And `json.dumps`{:.language-py} was also interesting because by default, it can output `NaN`{:.language-js} and `Infinity`{:.language-js}, neither of which are valid JSON, so most other parsers like JavaScript would fail on it. I've always wanted to try exploiting this difference in a CTF.

### Development

I decided to have a JavaScript server that would run Python to evaluate an arithmetic expression, and it would only reveal a flag in the special case when `JSON.parse`{:.language-js} fails. And since I started working on the challenge after the CTF started, I knew that open source challenges were much preferred, so I designed the server knowing all its files would be published as attachments to the challenge.
...
It's only if Python runs without error that it'll try to parse it. I also made sure to check that it starts and ends with curly braces, in case Python prints out some weird non-JSON like `None`{:.language-py}, an empty string, `null`{:.language-js}, etc.
...
My solution is extremely straightforward: `1 / 5e-324`{:.language-js}.
...
I was hoping to take advantage of another Python quirk here. Python is unusual compared to pretty much all other programming languages in that float division by zero is a runtime error, so you can't just do `1.0 / 0.0`{:.language-py}. However, dividing by the smallest positive double-precision floating point value still produces `inf` in Python. You can easily get this value by running `Number.MIN_VALUE`{:.language-js} in JavaScript.
...
The most common issue people ran into with this challenge is that they managed to get Infinity, but on the JavaScript side. Their approach was to multiply together a bunch of large ints, like `99999 * 99999 * ... * 99999`{:.language-js}.

The reason why this doesn't work is because JavaScript will serialize each of these numbers as `99999`{:.language-js}, rather than `99999.0`{:.language-js}, because that's how those numbers are stringified in JS. However, Python has a distinction between the two when parsing; `99999`{:.language-js} becomes an `int`{:.language-py}, while `99999.0`{:.language-js} becomes a float.

Python `int`{:.language-py} objects are equivalent to `BigInt`{:.language-js} in JavaScript, so they can be multiplied ad nauseam without producing Infinity. Python will happily turn the int product back into a super long string of digits in JSON, which JavaScript can parse without throwing a syntax error. However, it'll then cast the string of digits to a number (i.e. a float), so it becomes Infinity.
...
The simplest fix to that problem would just be to force Python to cast the int to a float by multiplying by `1.0`{:.language-js}.

Write ups:

- [@ky28059](https://gist.github.com/ky28059/e8fc98d5b0be999b1409cc80a9077c54): "This challenge is pretty trivial if you know about how Python's `json.dumps`{:.language-py} is JSON spec noncompliant. In particular, Python will successfully serialize `NaN`{:.language-js} and `Infinity`{:.language-js}, despite neither of those values being valid JSON."
...
[web/calculator](#webcalculator) is my favorite challenge and relies on Python's `json.dumps`{:.language-py} producing invalid JSON for `inf` and `nan` values. I learned that providing source code makes the challenge more approachable and enjoyable.

...
Computer science isn't the only capped major either.
As an undeclared student,
I can switch to any _uncapped_ major.
Based on the
...
- **Data Science** (Data Sci, DSC),
  which is capped, strangely,
  because from the looks of it,
  many CS rejects end up as data science majors.
- I think there's also the bioinformatics major.
  I don't know which one,
  but I know one person who used it as an alternative.
...
(which is a different fun story for later).
...
they'll let you off the waitlist before anyone else
and shove you into the course.
...
Also, they're getting rid of it next year,
so it'll become impossible to get into CS starting then.
...
and then let students in ranked by that GPA.
...
- **PHYS 2A** (mechanics)
  ...
  exempting the wrong courses,
  because I also took AP Physics 1,
  and they also misread "Physics" as "Psych"
  and gave me credit for the wrong course
  (which happened to satisfy my Sixth GE's).
  I asked them to correct it just in case,
  and also I only needed 2A/2B credit,
  not credit for the other introductory physics courses.

- **CSE 11**
  (AP CS A, the stack and heap, and file reading),
  which I _don't_ get credit for from AP CS A,
  so I took it this fall quarter;
  ...
  I think I can talk more about it at the end of this Longer Tweet.
...
considering how many Math CS majors there are.
...
that I'll have to submit a quarter-by-quarter plan
...
(eg computer science, computer engineering, etc.),
...
(an Instagram story, a Discord status)
...
while also showing more than just the tip of the iceberg.

## Appendix: an aside on CSE 11

Here's a chart of the lower division CSE course requirements
for a CSE major and Math CS (and probably other majors too).
...
what matters more are the concepts taught
and how easily you can learn a new programming language.
...
but I'd like to warn that I thus have a skewed perception of 8B.
...
and our school taught significantly more
...
coming to UCDS for one still have to go through the course.

...
Here, if it finds `suicidepreventionlifeline.org` in the user's URL, it'll short circuit and allow the web page request through. Interestingly, although it's checking for a domain name here, it's checking it in `e.url`{:.language-js}. Indeed, adding `?suicidepreventionlifeline.org` to the end of the URL includes it in `e.url`{:.language-js}, so `-1 != e.url.indexOf("suicidepreventionlifeline.org")`{:.language-js} evaluates to true, allowing any site to be let through.

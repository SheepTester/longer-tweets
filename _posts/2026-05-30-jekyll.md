...
Despite the lack of arbitrary scripting, Jekyll and its [plugins supported by GitHub Pages][plugins] offer far more than just basic Markdown rendering and HTML templating.
...
[^md-xss]:
    ...
    - Markdown is essentially a [superset of HTML][md-html]. Users can easily inject arbitrary code with a `<script>`{:.language-html} tag because [`<script>`{:.language-html} tags are explicitly valid Markdown][md-script].

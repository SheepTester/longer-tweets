...
I decided to only do the web challenges
...
- [Ishihara test++](#a-svgcss-based-challenge)
- [Hu*Mongo*us Mistake](#mongodb-admin-ctf)
- [Lots of Logs](#logs-ctf)
- [JaWT that down!](#jawtjwt-ctf)
- [SymCalc.js](#ctf-node-calculator-escape-vm-codegeneration-false)
- [Internprise Encryption](#inverse-encrypting)
- [Oracle](#more-inverse-encrypting)
- Failures
  - [CURL Up and Read](#curl-get-environment-variable-ctf)
  - [Gullible by Dovesign](#ctf-upload-file)
...
There were five people in total on my team, but I have no idea what they did, so I won't mention their challenges here.
...
After the Ishihara test, I decided to try tackling all the web challenges, but they weren't very trivial, so I ended up investigating all of them in parallel, switching to a different challenge when I got stuck for long enough. This challenge was the next one I solved.
...
_The flag-shaped shell is treated as a special shell in the page code, and redirects to `/buyflagshell?shell=flag-shaped-shell` rather than `/buyshell`. The shell kind of makes me uncomfortable for some reason._
...
The `dangerouslySetInnerHTML` looked pretty sus, but it's pretty unlikely that the JavaScript is rendered server side, so looking back, it's probably just a hack to insert comments into the HTML for non-React people.
...
One of the "easy" web challenges stumped me because it seemed to just be a static site.
...
I fiddled around with it and tried using a [secret cracker](https://lmammino.github.io/jwt-cracker/) I found on npm, but I didn't really get anywhere since it wouldn't accept my new token. I got stuck.
...
Only `/s` was giving `d` now. `/sd` still 404'd, and `/s?d` still gave `d`. Eventually, I tried `/s/d`, which gave `c`. Exciting; it's the next character in the sequence of the beginning of the flags: `sdctf{`.
...
One of my teammates was looking at a non-web challenge (it was in the jail category) that involved JavaScript while we were at Burger King last night. I figured that he would be busy enough that I could just steal it from him. All JavaScript belongs to me.
...
Ah, bangs (`!`) aren't in the allowlist. I noticed that numbers and `=` were allowed, so I decided to just write the JavaScript myself, using [JSFuck's code](https://github.com/aemkei/jsfuck/blob/master/jsfuck.js) for reference. This wasn't too bad for me; I have tried to re-derive JSFuck's characters before, so I was pretty comfortable with this.
...
_I generated the `fromCharCode` part using `[...'return process.env.FLAG'].map(a => `fromCharCode(${a.codePointAt()})`).join(' + ')`._
...
I tried looking up articles who knew how to escape out of `codeGeneration: false`, but no one seemed to really talk about it; I guess other CTFs are easier and just leave it on.
...
However, I found that `this` was the empty context object, and doing `.constructor.constructor()` was acceptable.
...
_I had originally used `constructor` as a variable that stored the string `'constructor'` for readability, but because it's a global variable, it got stored in the context object `this`, so `this.constructor` got overwritten to the string rather than `Object`. Oops._
...
I was bored and browsed through the other non-web challenges near the beginning, and I found a challenge that involved JavaScript, so of course I had to do it.
...
The text files were all encrypted in jarbly mess, so I assumed that maybe I had to use frequency analysis or something based on the other text files as examples.
...
When I saw it, I assumed that I could probably find a way to reverse it. First, I simplified the encrypt function (and called it `simp` because it's funny).
...
2. ```js
   ...
   Inverting is just the same thing but decrementing instead of incrementing.
...
3. ```js
   ...
   The inverse operation of XOR seems to just be XOR it with the same number again. That's because XOR flips the other bit if the bit is 1 and leaves it alone if it's 0, and to undo a flip, I just need to flip it again.
...
4. ```js
   ...
   In JavaScript, `~` was useful because it turns 0 into -1, a truthy value, and -1 into 0, a falsy value, so you can use `~arr.indexOf(value)` as a boolean value for whether `value` is in `arr`. It can be thought of as equivalent to `-1 - x`.
...
7. ```js
   ...
   Logging `x >> 31`, it seems it's always `-1`, so I just assumed it was always the case. Apparently this assumption works.
...
8. ```js
   ...
   `i` is an integer, so `i % 1` is always zero. Bitwise OR'ing any number by 0 truncates it because it casts it to an i32, and OR'ing anything with false is itself. Since `(Date.now() % 1000) / 1000` is always less than 1, it'll always get truncated to 0. `x |= 0` is also a no-op.

Here's my resulting decryption function, which I turned into a Deno program.
...
One of my teammates deobfuscated a .class file from one of the challenges but didn't know how to deal with the bitshifts. I have no idea how she converted it to Java.
...
I wrote the decryption algorithm in JavaScript because I only know JavaScript. I pasted `CHECK`, which seems to be the encrypted flag, into an `Int8Array`, which was the JS equivalent for Java `byte[]` (since Java `byte`s are signed); I assumed `Byte.MIN_VALUE` is -128.

I started with the third pass, which seemed fairly simple. It was only dependent on `i`, and it seemed to add an expression dependent only on `i` to `numbers[i]`, so its inverse would just be to decrement by that expression based on `i`.

```js
check[i] -= 7 * i * i + 31 * i + 127 + (i % 2)
```

The second pass seemed a bit more spicy, so I inverted the first pass first, which was about as ceremonious as the third. I had to look up the operator precedence for bitwise XOR (`^`), and it seems to bind really loose, so the expression was equivalent to `numbers[i] = numbers[i] ^ (...)`, i.e. `numbers[i] ^= ...`. The inverse of `^`, as in the [previous challenge](#inverse-encrypting), is still `^`.
...
To invert this, I need to shift the current byte left by four bits, which would discard the foreign bits `EFGH`, and then merge it with the first four bits of the next byte, which has the last four bits of the original byte `MNOP`. I made a clone of the array because the bytes involve other bytes in the array, though I'm not sure if that was really necessary.
...
I wasn't really sure what the cause was, so I tried changing the `Int8Array` to a `Uint8Array`. It worked.
...
Alas, I wasn't able to do all the web challenges. I made some progress with them, but then I got stuck. Based on fellow participants in the CTF server, I'm including their solutions here for anyone who happens upon this post from Google.
...
They recommend reading [`man 5 proc`](https://man7.org/linux/man-pages/man5/proc.5.html) for more about `/proc`. I'll have to keep this in mind next time. This is big brain and a shame that Google didn't show me this. Hopefully this write-up might show up in the search results next time.
...
- `The image overwrite me.jpg does not seem to be a bird.` if the image has an EXIF `Description` tag that isn't a hardcoded species name in their database.

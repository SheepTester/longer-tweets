...
I edited the `<style>`{:.language-html} tag in inspect element, changing one class's `fill` colour to a different colour at a time. Some of the classes seemed to form a message, while others are just random. Here's flag.svg, where each class has a different colour.
...
I assumed the server was just checking `req.body.password.length`{:.language-js}, so I just added a `length` property to `password`:
...
Unfortunately, despite using the `Function`{:.language-js} instance from the parent environment, the VM still prevented me from using its constructor to evaluate the string as JS.

> EvalError: Code generation from strings disallowed for this context

Annoying!

I tried looking up articles ("node vm codeGeneration false ctf," "codeGeneration false node vm") who knew how to escape out of a VM with `codeGeneration`{:.language-js} set to false, but no one seemed to really mention that option. Other CTFs that did use a Node VM didn't use the option. I guess other CTFs are easier and just leave code generation on.
...
The VM was indeed displeased with `.constructor.constructor()`{:.language-js}.
...
However, I found that `this`{:.language-js} was the empty context object, and doing `.constructor.constructor()`{:.language-js} on it was acceptable.
...
Conveniently, identifiers are allowed in the calculator, so I entered the equivalent of `this.constructor.constructor('return process.env.FLAG')()`{:.language-js} into the calculator, which gave me the flag. 🎉
...
After the CTF, I would find out that I had overcomplicated this; I could instead have used regex literals in lieu of string literals and used `with`{:.language-js} statements to get properties. Whatever.
...
1. ```js
   ...
   `% 128`{:.language-js} isn't a huge concern because the characters are all presumably ASCII characters, so their code values should be less than 128.

   To invert this, I can subtract `i * 15`{:.language-js} and then mod by 128 again.
...
3. ```js
   ...
   The inverse operation of XOR seems to just be XOR it with the same number again (i.e. `(a ^ b) ^ b === a`{:.language-js}). That's because XOR flips the other bit if the bit is 1 and leaves it alone if it's 0, and to undo a flip, I just need to flip it again.
...
4. ```js
   ...
   `x ^ ~0x0`{:.language-js} is the same as `x ^ -1`{:.language-js}. `-1`{:.language-js} as an i32 is just 32 ones, so XOR'ing it with `x`{:.language-js} will flip all of its bits. The bitwise NOT operator `~`{:.language-js} does the same thing.

   In JavaScript, `~`{:.language-js} was useful because it turns 0 into -1, a truthy value, and -1 into 0, a falsy value, so you can use `~arr.indexOf(value)`{:.language-js} to convert the index to a boolean value for whether `value`{:.language-js} is in `arr`{:.language-js}. `~x`{:.language-js} can be thought of as equivalent to `-1 - x`{:.language-js}.

   I tried removing `& 255`{:.language-js} because I assumed that the ASCII values would be less than 255, but that changed the encrypted result, so I added it back.

   I wasn't really sure how to invert `& 255`{:.language-js}, so I ignored it for now. To invert the bitwise NOT, I can just NOT it again.
...
5. ```js
   ...
   I assumed bitwise AND is associative, so `(a & b) & c`{:.language-js} is the same as `a & (b & c)`{:.language-js}, so I can simplify `0xb9 & 0x46`{:.language-js} to 0. AND'ing a number with 0 makes all of its bits false (anything and false is false), so `(Math.random() * 5) & 0`{:.language-js} is always zero, and the line does nothing. It shouldn't affect the result anyways since `encrypt`{:.language-js} seems to be deterministic.
...
6. ```js
   ...
   The `0x2cd +`{:.language-js} is part of the condition, so it's always truthy, so `0x2cd + ((i ^ 0x44) % 0x2 === 0) ? 0x3 : 0x0`{:.language-js} always evaluates to 3.

   To invert this, I have to invert the `~`{:.language-js} first then the `^ 3`{:.language-js}.
...
7. ```js
   ...
   Logging `x >> 31`{:.language-js}, it seems it's always `-1`{:.language-js}, so I just assumed it was always the case. (This makes sense because `x`{:.language-js} was NOT'd---effectively negated---three times before this, and `x`{:.language-js} is originally positive.)

   To invert this, I invert the `~`{:.language-js} then add 1.
...
8. ```js
   ...
   Modding `Date.now()`{:.language-js} by 1000 keeps it in the range [0, 1000). Dividing it by 1000 makes the interval [0, 1).

   `i`{:.language-js} is an integer, so `i % 1`{:.language-js} is always zero. Bitwise OR'ing any number by 0 just truncates it because it casts it to an i32, and OR'ing anything with false is itself. Since `(Date.now() % 1000) / 1000`{:.language-js} is always less than 1, it'll always get truncated to 0. `x |= 0`{:.language-js} is also a no-op.

Here's my resulting decryption function, which I turned into a Deno program (I really do not want to `require('fs')`{:.language-js} and deal with Node spiciness).
...
I wrote the decryption algorithm in JavaScript because I only know JavaScript. I pasted the contents of `CHECK`{:.language-java}, which seems to be the encrypted flag, into an `Int8Array`{:.language-js}, which was the JS equivalent for Java `byte[]`{:.language-java} (since Java `byte`{:.language-java}s are signed); I assumed `Byte.MIN_VALUE`{:.language-java} is -128.

I worked backwards, starting with the third pass, which seemed fairly simple. It was only dependent on `i`{:.language-js}, and it seemed to add an expression dependent only on `i`{:.language-js} to `numbers[i]`{:.language-js}, so its inverse would just be to decrement by that expression based on `i`{:.language-js}.

```js
check[i] -= 7 * i * i + 31 * i + 127 + (i % 2)
```

I decided to invert the first pass next, which was about as ceremonious as the third, because the second pass seemed a bit more spicy. I had to look up the [operator precedence](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Operator_Precedence#table) for bitwise XOR (`^`{:.language-js}), and it seems to bind really loose (looser than `===`{:.language-js}), so the expression was equivalent to `numbers[i] = numbers[i] ^ (...)`{:.language-js}, i.e. `numbers[i] ^= ...`{:.language-js}. The inverse of `^`{:.language-js}, as I discussed the [previous challenge](#inverse-encrypting), is still `^`{:.language-js}.
...
The second pass seemed to involve other bytes, not just `numbers[i]`{:.language-js}. I saw `... << 4 | ... >> 4`{:.language-js}, so it seemed like it was chopping the byte (8 bits) in half (4-bit pieces), swapping their positions using bit shifts, and then merging them back together with the bitwise OR. It wasn't so simple, though; `numbers[((i + 42 - 1) % 42)]`{:.language-js} gets the previous byte, wrapping around to the end of the array for the first byte.
...
I wasn't really sure what the cause was, so I tried changing the `Int8Array`{:.language-js} to a `Uint8Array`{:.language-js}; maybe JS bitwise operations are wacky when it comes to negative numbers. It worked.
...
The principles of JSFuck are knowing what each character is used for in JavaScript syntax and understanding JavaScript type casting. `+`{:.language-js} is used for addition, concatenation, and casting to a number; `[]`{:.language-js} are used for creating arrays and getting properties from objects; and `()`{:.language-js} is used for grouping expressions and calling functions.

From just those characters, I can create strings from some JavaScript keywords that I can get individual letters from. This is necessary because quotes aren't allowed. For example, `[][[]]`{:.language-js} (`[][0]`{:.language-js} also works in SymCalc.js) produces **`undefined`{:.language-js}**. The empty array `[]`{:.language-js} has no such property `''`{:.language-js} (an empty string, which is what an empty array becomes when [casted to a string](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/toString)) or `0`{:.language-js}, so it returns `undefined`{:.language-js}.

Even though exclamation marks (`!`) aren't allowed in SymCalc.js, equal signs (`=`) and digits are, so I can also get **`true`{:.language-js}** from `0 === 0`{:.language-js} and **`false`{:.language-js}** from `0 === 1`{:.language-js}.
...
This is because an empty array is not a number, so JavaScript does string concatenation instead of addition. It casts both operands to a string, and `undefined`{:.language-js}, `false`{:.language-js}, etc. become `'undefined'`{:.language-js}, `'false'`{:.language-js}, etc., while the empty array becomes an empty string.

Then, to get individual letters from these strings, you can just index them with numbers: `([][[]] + [])[0]`{:.language-js} becomes `(undefined + [])[0]`{:.language-js} then `('undefined' + '')[0]`{:.language-js}, `'undefined'[0]`{:.language-js}, which gets the first letter, `'u'`{:.language-js}. SymCalc.js also allows using letters and equal signs, so for convenience and readability, I stored these letters into variables of the same name. Here are all the letters I can get so far:
...
`this`{:.language-js} is an empty object, and its constructor is [`Object`{:.language-js}](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object), which is the superclass for all JavaScript objects. You can get the constructor function of almost any JavaScript object through its [`constructor`{:.language-js} property](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/constructor). `Object` is a function, so its constructor is [`Function`{:.language-js}](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Function). [Calling `Function`{:.language-js} with a string](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Function/Function) "casts" it to a function, which lets me run a string as JavaScript code. Finally, the final `()` calls the created function to get the challenge flag, which is stored in `process.env.FLAG`{:.language-js}, as hinted by the given code.
...
However, the alphabet I have so far is incomplete. I still need `'c'`{:.language-js} and `'o'`{:.language-js} for the constructor and the characters in `' .FLAG'`{:.language-js} for the string I pass to `Function`{:.language-js}.

I'll focus on `'c'`{:.language-js} and `'o'`{:.language-js} first. JSFuck obtains these by casting a function to a string. When you [cast a function to a string](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Function/toString), such as `alert + ''`{:.language-js}, it returns the source code of the function, or `[native code]`{:.language-js} if the function is built-in. Examples:
...
That `function`{:.language-js} keyword (and "native code") has the C and O that I need, so I just need to get access to some function.

JSFuck usually goes about this by using an array method. It initially used [`Array.prototype.fill`{:.language-js}](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/fill), but now uses the new [`Array.prototype.flat`{:.language-js}](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/flat). So, to get a string that starts with `function`{:.language-js}, I need to do `[]['fill'] + []`{:.language-js} or `[]['flat'] + []`{:.language-js}. JSFuck uses these method names because they're short and only use letters that one can get so far.
...
Finally, I can get `'constructor'`{:.language-js}. I initially stored it in a variable named `constructor`{:.language-js}, but because of the way I declared it, it gets stored in the context object, so `this.constructor`{:.language-js} was `'constructor'`{:.language-js} not `Object`. To avoid this, I named the variable `constructo`{:.language-js}.

```js
constructo = c + o + n + s + t + r + u + c + t + o + r // 'constructor'
```

What about the other characters? While I can obtain the space character from the function source string `func`{:.language-js}, that approach won't work for characters like the dot (`.`). I would have to use [`String.fromCharCode`{:.language-js}](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/fromCharCode) or [`String.fromCodePoint`{:.language-js}](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/fromCodePoint), which can produce any character given its Unicode code value. [`String`{:.language-js}](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String) is the constructor of any string (`''.constructor`{:.language-js}, or `([] + [])['constructor']`{:.language-js}), but to get its `fromCharCode`{:.language-js} method, I need an H, M, and capital C.

The [way JSFuck approaches `C`](https://github.com/aemkei/jsfuck/blob/1f02651e98da1c13aef76d609755a2568a1aa6b4/jsfuck.js#L54) is pretty genius. It uses the built-in function [`escape`{:.language-js}](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/escape) to [URL-encode a `<` character to `%3C`](https://en.wikipedia.org/wiki/Percent-encoding#Character_data), and to get a `<` character, it uses one of the [string methods that insert HTML tags around the string](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String#html_wrapper_methods) (for example, `'wow'.big()`{:.language-js} returns `<big>wow</big>`{:.language-html}). JSFuck uses [`.italics()`{:.language-js}](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/italics), so I used it as well. To get the `escape`{:.language-js} function, JSFuck uses `Function`{:.language-js} to create a function that returns `escape`{:.language-js}, but that's not necessary for SymCalc.js because it allows letters.

```js
C = escape(([] + [])[i + t + a + l + i + c + s]())[2] // 'C' from '%3Ci%3E%3C/i%3E', an escaped '<i></i>'
```

To get `m`, JSFuck uses [`Number`{:.language-js}](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Number) (the constructor for numbers):

```js
m = ((0)[constructo] + [])[11] // 'm', from 'function Number() { [native code] }'
```

Then for `h` and a lot of the other letters, JSFuck uses [`Number.prototype.toString`{:.language-js}](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Number/toString) to convert a number into a base that uses `h`, kind of like how hexadecimal uses A--F for its extra six digits. 17 in base 10 is H in bases larger than base 17, so `(17).toString(36)`{:.language-js} produces `'h'`{:.language-js}. To get `'toString'`{:.language-js}, JSFuck concatenates `t` and `o` with [`String.name`{:.language-js}](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Function/name), the name of the `String`{:.language-js} function.
...
Now, I can get `String.fromCharCode`{:.language-js}:

```js
fromCharCode = Str[f + r + o + m + C + h + a + r + C + o + d + e]
```

I got lazy, so I just used `fromCharCode`{:.language-js} for the rest of the characters. I wrote code to generate code that returns code for running code:
...
Finally, I can write `this['constructor']['constructor']('return process.env.FLAG')()`{:.language-js} without quotes or dots, which you can find in the main [SymCalc.js write-up](#ctf-node-calculator-escape-vm-codegeneration-false).

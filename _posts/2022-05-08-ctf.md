...
_I had originally used `constructor` as a variable that stored the string `'constructor'` for readability, but because it's a global variable, it got stored in the context object `this`, so `this.constructor` got overwritten to the string rather than `Object`, which confused me. Oops._
...
JSFuck only uses the following six characters: `!+[]()`. In SymCalc.js, all but the bang (`!`) are permitted, but because the bang is often used, it means traditional JSFuck can't be used directly for SymCalc.js.
...
- `+` is used for addition, concatenation, and converting to a number

  - `a + b` adds numbers (if both operands are numbers); otherwise, it'll try to concatenate them as a string, and if an operand isn't a string, it'll cast them to a string.
  - `+a` is the unary plus operator. It's kind of like negation (`-a`) except it doesn't negate the number. It seems pretty useless, but because, like `-a`, `+a` only makes sense for numbers, it casts `a` to a number. In JavaScript, `+a` is idiomatic for converting something to a number:
...
- `[]` is used both for creating arrays and getting properties of an object

  - `[1, 2, 3]` is an array literal. It creates an array containing 1, 2, and 3. In JSFuck and SymCalc.js, this isn't super useful because commas aren't allowed, but you can create empty arrays `[]` or single-item arrays (e.g. an array containing just an empty array `[[]]`).
  - `object[expression]` gets the given property from `object`. For example, `console.log` is equivalent to `console['log']`, and in general, `object.property` is equivalent to `object['property']`. You can put any expression in the square brackets, and JavaScript will cast the return value to a string and look up

    You can use this bracket notation directly on an array literal.
...
- `()` is used for grouping expressions and for calling functions

  - Like in math, you can use parentheses to group expressions. For example, normally, `a + b * c` does the multiplication first, but you can do `(a + b) * c` to do addition first.
  - Using parentheses after an expression will try to call the expression as a function with the given arguments: `func()` or `func(arg)`.
